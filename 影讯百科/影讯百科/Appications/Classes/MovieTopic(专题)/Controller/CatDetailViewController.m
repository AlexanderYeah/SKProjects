...
@interface CatDetailViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
/** collectionView */
...
{   // 提示用户
    ...
    // 2 进行请求
    NSString *urlStr = [NSString stringWithFormat:@"http://api.maoyan.com/mmdb/search/movie/source/list.json?src=%ld&limit=20&offset=20",self.index];
...
- (void)reloadMoreData
{
    ...
    NSString *urlStr = [NSString stringWithFormat:@"http://api.maoyan.com/mmdb/search/movie/source/list.json?src=%ld&limit=%d&offset=20",self.index,pageIndex];
    // 2 进行请求
    [manger GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
