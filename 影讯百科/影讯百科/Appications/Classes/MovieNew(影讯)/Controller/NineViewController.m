...
- (void)requestDataFromNet
{
    ...
    NSString *urlStr = @"http://movie.douban.com/j/search_subjects?type=movie&tag=悬疑&sort=recommend&page_limit=20&page_start=0";
...
- (void)reloadMoreData
{
    ...
    // 1 创建网络请求对象
    AFHTTPSessionManager *manger = [AFHTTPSessionManager manager];
    NSString *urlStr =[NSString stringWithFormat: @"http://movie.douban.com/j/search_subjects?type=movie&tag=悬疑&sort=recommend&page_limit=%d&page_start=0",page];
    NSString *encodeStr = [urlStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
