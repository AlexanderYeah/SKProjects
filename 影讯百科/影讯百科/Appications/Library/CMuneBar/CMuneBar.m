...
-(void)addSubviews{
    ...
    [mainButton setBackgroundColor:kRGBColor(217, 86, 113)];
    
    [mainButton setTitle:@"分享" forState:UIControlStateNormal];
    [mainButton setTitle:@"取消" forState:UIControlStateSelected];
...
-(NSArray *)items{
    if (_items == nil) {
        ...
        for (int i = 0; i < self.itemsImages.count; i++) {
            ...
            CMuneItem *item = [CMuneItem muneItemWithSize:CGSizeMake(38, 38) image:image heightImage:nil target:self action:@selector(tapItem:)];
            item.tag = 100 + i;
